...
```{r}
###Import File
atl_violent <- read.csv("atl_violent_final_v2.csv", head = TRUE)
names(atl_violent)[1] <- 'Date'
atl_v <- ts(atl_violent$occurance_count, start = 2009, freq = 365)

atl_prop <- read.csv("atl_prop_final_v2.csv", head = TRUE)
names(atl_prop)[1] <- 'Date'
atl_p <- ts(atl_prop$occurance_count, start = 2009, freq = 365)

nyc_violent <- read.csv("nyc_violent_final_v3.csv", head = TRUE)
...
joined_df <- merge(atl_prop, atl_violent, by.x = "Date", by.y = "Date", all.x = TRUE, all.y = TRUE)
names(joined_df)[2]="Atlanta Property Crimes"
names(joined_df)[3]="Atlanta Violent Crimes"

joined_df <- merge(joined_df, nyc_violent, by.x = "Date", by.y = "Date", all.x = TRUE, all.y = TRUE)
names(joined_df)[4]="NYC Violent Crimes"

joined_df <- merge(joined_df, nyc_prop, by.x = "Date", by.y = "Date", all.x = TRUE, all.y = TRUE)
names(joined_df)[5]="NYC Property Crimes"

joined_df=joined_df[order(as.Date(joined_df$Date, format="%m/%d/%Y")),]

cor(joined_df[, c('Atlanta Property Crimes', 'Atlanta Violent Crimes', 'NYC Violent Crimes', 'NYC Property Crimes')])
```
...
time.pts = c(1:length(atl_v.tr))
time.pts = c(time.pts - min(time.pts))/max(time.pts)

#Splines Trend Estimation
gam.fit = gam(atl_v.tr~s(time.pts))
atl_v.tr.fit.gam = ts(fitted(gam.fit),start=2009,frequency=365)

##Is there a trend? 
ts.plot(atl_v.tr,ylab="Number of Crimes", main = "ATL Violent Crimes - Splines")
grid()
lines(atl_v.tr.fit.gam,lwd=2,col="red")
```

```{r}
# Splines Residuals + Residuals ACF
dif.fit.gam = atl_v.tr - atl_v.tr.fit.gam
...
model.anova = dynlm(atl_v.tr~season(atl_v.tr))
#summary(model.anova)

## Plot
ts.plot(atl_v.tr,ylab="Number of Crimes", main = "ATL Violent Crimes - ANOVA Seasonality")
grid()
...
###SUMMARY
harmonic.1 = dynlm(atl_v.tr~harmon(atl_v.tr))
summary(harmonic.1)

harmonic.2 = dynlm(atl_v.tr~harmon(atl_v.tr,2))
summary(harmonic.2)
...
###Harmonic Plot
## Plot
ts.plot(atl_v.tr,ylab="Number of Crimes", main = "ATL Violent Crimes - Harmonic Seasonality")
grid()
...
diff.atl_v.tr = diff(atl_v.tr)
ts.plot(diff.atl_v.tr, ylab = "Differenced Data", main = "ATL Violent Crimes - Differenced Order 1")
abline(h=0, col='red')
acf(diff.atl_v.tr, xlim=c(0.3,4), lag.max = 365 * 4, main = "ATL Violent Crimes - Differenced Order 1 ACF")
### SEE HW2 - Q2 FOR EXPLANATION
```

```{r}
###DIFFERENCED DATA - ORDER 2 - TRANSFORMED
diff.atl_v.tr = diff(atl_v.tr, differences = 2)
ts.plot(diff.atl_v.tr, ylab = "Differenced Data", main = "ATL Violent Crimes - Differenced Order 2")
abline(h=0, col='red')
acf(diff.atl_v.tr, xlim=c(0.2,4), lag.max = 365 * 4, main = "ATL Violent Crimes - Differenced Order 2 ACF")
### SEE HW2 - Q2 FOR EXPLANATION
```

```{r}
###DIFFERENCED DATA - TRANSFORMED - LAG 7
diff.atl_v.tr = diff(atl_v.tr, 7)
ts.plot(diff.atl_v.tr, ylab = "Differenced Data", main = "ATL Violent Crimes - SqRt Differenced Lag 7")
abline(h=0, col='red')
acf(diff.atl_v.tr, xlim=c(0.2,4),lag.max = 365 * 5, main = "ATL Violent Crimes - SqRt Differenced Lag 7 ACF")
```
...
###TREND ESTIMATION - SPLINES
# Convert X-Axis to 0-1 Scale
time.pts = c(1:length(atl_p.tr))
time.pts = c(time.pts - min(time.pts))/max(time.pts)

#Splines Trend Estimation
gam.fit.3 = gam(atl_p.tr~s(time.pts))
atl_p.fit.gam = ts(fitted(gam.fit.3),start=2009,frequency=365)

##Is there a trend? 
ts.plot(atl_p.tr,ylab="Number of Crimes", main = "ATL Property Crimes - Splines")
grid()
lines(atl_p.fit.gam,lwd=2,col="red")
```
```{r}
plot(atl_p.fit.gam, type = "l", col = "red", xlab = "Year", ylab = "Number of Crimes", ylim=c(2.5,10.5))
lines(atl_v.tr.fit.gam, col = "blue", type = "l")
legend("topright", legend=c("ATL Property Spline", "ATL Violent Spline"),
...
dif.fit.gam4 = atl_p.tr - atl_p.fit.gam
...
## Estimate seasonality using ANOVA approach
model.anova2 = dynlm(atl_p.tr~season(atl_p.tr))
#summary(model.anova)

##Plot
ts.plot(atl_p.tr,ylab="Number of Crimes", main = "ATL Property Crimes - ANOVA Seasonality")
grid()
...
###SEASONALITY ANALYSIS - HARMONIC
###SUMMARY
harmonic.3 = dynlm(atl_p.tr~harmon(atl_p.tr))
summary(harmonic.3)

harmonic.4 = dynlm(atl_p.tr~harmon(atl_p.tr,2))
summary(harmonic.4)
...
###Harmonic Plot
## Plot
ts.plot(atl_p.tr,ylab="Number of Crimes", main = "ATL Property Crimes - Harmonic Seasonality")
grid()
...
diff.atl_p.tr = diff(atl_p.tr)
ts.plot(diff.atl_p.tr, ylab = "Differenced Data", main = "ATL Property Crimes - Differenced Order 1")
abline(h=0, col='red')
acf(diff.atl_p.tr, lag.max = 365 * 4, main = "ATL Property Crimes - Differenced Order 1 ACF")
...
diff.atl_p2 = diff(atl_p.tr, lag = 7)
...
###TREND ESTIMATION - SPLINES
# Convert X-Axis to 0-1 Scale
time.pts = c(1:length(nyc_v.tr))
time.pts = c(time.pts - min(time.pts))/max(time.pts)

#Splines Trend Estimation
gam.fit = gam(nyc_v.tr~s(time.pts))
nyc_v.tr.fit.gam = ts(fitted(gam.fit),start=2009,frequency=365)

##Is there a trend? 
ts.plot(nyc_v.tr,ylab="Number of Crimes", main = "NYC Violent Crimes - Splines")
grid()
lines(nyc_v.tr.fit.gam,lwd=2,col="red")
```

```{r}
# Splines Residuals + Residuals ACF
dif.fit.gam = nyc_v.tr - nyc_v.tr.fit.gam
...
## Estimate seasonality using ANOVA approach
model.anova = dynlm(nyc_v.tr~season(nyc_v.tr))
#summary(model.anova)

## Plot
ts.plot(nyc_v.tr,ylab="Number of Crimes", main = "NYC Violent Crimes - ANOVA Seasonality")
grid()
lines(fitted(model.anova),lwd=2,col="red")
```
...
###SEASONALITY ANALYSIS - HARMONIC
###SUMMARY
harmonic.1 = dynlm(nyc_v.tr~harmon(nyc_v.tr))
summary(harmonic.1)

harmonic.2 = dynlm(nyc_v.tr~harmon(nyc_v.tr,2))
summary(harmonic.2)
...
###Harmonic Plot
## Plot
ts.plot(nyc_v.tr,ylab="Number of Crimes", main = "NYC Violent Crimes - Harmonic Seasonality")
grid()
...
```{r}
###DIFFERENCED DATA - ORDER 1
diff.nyc_v.tr = diff(nyc_v.tr)
ts.plot(diff.nyc_v.tr, ylab = "Differenced Data", main = "NYC Violent Crimes - Differenced Order 1")
abline(h=0, col='red')
acf(diff.nyc_v.tr, lag.max = 365 * 4, main = "NYC Violent Crimes - Differenced Order 1 ACF")
```

```{r}
###DIFFERENCED DATA - ORDER 2
diff.nyc_v.tr = diff(nyc_v.tr, differences = 2)
ts.plot(diff.nyc_v.tr, ylab = "Differenced Data", main = "NYC Violent Crimes - Differenced Order 2")
abline(h=0, col='red')
acf(diff.nyc_v.tr, lag.max = 365 * 4, main = "NYC Violent Crimes - Differenced Order 2 ACF")
```

```{r}
###DIFFERENCED DATA - ORDER 7
diff.nyc_v.tr = diff(nyc_v.tr, differences = 7)
ts.plot(diff.nyc_v.tr, ylab = "Differenced Data", main = "NYC Violent Crimes - Differenced Order 7")
abline(h=0, col='red')
acf(diff.nyc_v.tr, xlim=c(0.2,4), lag.max = 365 * 4, main = "NYC Violent Crimes - Differenced Order 7 ACF")
```
...
###TREND ESTIMATION - SPLINES
# Convert X-Axis to 0-1 Scale
time.pts = c(1:length(nyc_p.tr))
time.pts = c(time.pts - min(time.pts))/max(time.pts)

#Splines Trend Estimation
gam.fit = gam(nyc_p.tr~s(time.pts))
nyc_p.fit.gam = ts(fitted(gam.fit),start=2009,frequency=365)

##Is there a trend?
ts.plot(nyc_p.tr,ylab="Number of Crimes", main = "NYC Property Crimes - Splines")
grid()
lines(nyc_p.fit.gam,lwd=2,col="red")
```

```{r}
plot(nyc_p.fit.gam, type = "l", col = "red", xlab = "Year", ylab = "Number of Crimes", ylim=c(14,23))
lines(nyc_v.tr.fit.gam, col = "blue", type = "l")
legend("topright", legend=c("NYC Property Spline", "NYC Violent Spline"),
       col=c("red", "blue"), lty=c(1, 1), lwd=2)
```

```{r}
# Splines Residuals + Residuals ACF
dif.fit.gam = nyc_p.tr - nyc_p.fit.gam
...
###SEASONALITY ANALYSIS - ANOVA
## Estimate seasonality using ANOVA approach
model.anova = dynlm(nyc_p.tr~season(nyc_p.tr))
#summary(model.anova)

##Plot
ts.plot(atl_p.tr,ylab="Number of Crimes", main = "NYC Property Crimes - ANOVA Seasonality")
grid()
lines(fitted(model.anova),lwd=2,col="red")
```
...
###SEASONALITY ANALYSIS - HARMONIC
###SUMMARY
harmonic.1 = dynlm(nyc_p.tr~harmon(nyc_p.tr))
summary(harmonic.1)

harmonic.2 = dynlm(nyc_p.tr~harmon(nyc_p.tr,2))
summary(harmonic.2)
...
###Harmonic Plot
## Plot
ts.plot(nyc_p.tr,ylab="Number of Crimes", main = "NYC Property Crimes - Harmonic Seasonality")
grid()
lines(fitted(harmonic.2),lwd=2,col="red")
```
...
```{r}
###DIFFERENCED DATA - ORDER 1
diff.nyc_p.tr = diff(nyc_p.tr)
ts.plot(diff.nyc_p.tr, ylab = "Differenced Data", main = "NYC Property Crimes - Differenced Order 1")
abline(h=0, col='red')
acf(diff.nyc_p.tr, lag.max = 365 * 4, main = "NYC Property Crimes - Differenced Order 1 ACF")
### SEE HW2 - Q2 FOR EXPLANATION
```

```{r}
###DIFFERENCED DATA - ORDER 2
diff.nyc_p.tr = diff(nyc_p.tr, differences = 2)
ts.plot(diff.nyc_p.tr, ylab = "Differenced Data", main = "NYC Property Crimes - Differenced Order 2")
abline(h=0, col='red')
acf(diff.nyc_p.tr, lag.max = 365 * 4, main = "NYC Property Crimes - Differenced Order 2 ACF")
### SEE HW2 - Q2 FOR EXPLANATION
```

```{r}
###DIFFERENCED DATA - ORDER 7
diff.nyc_p.tr = diff(nyc_p.tr, differences = 7)
ts.plot(diff.nyc_p.tr, ylab = "Differenced Data", main = "NYC Property Crimes - Differenced Order 7")
abline(h=0, col='red')
acf(diff.nyc_p.tr, lag.max = 365 * 4, main = "NYC Property Crimes - Differenced Order 7 ACF")
```

```{r}
###DIFFERENCED DATA - LAG 7
diff.nyc_p.tr = diff(nyc_p.tr, 7)
ts.plot(diff.nyc_p.tr, ylab = "Differenced Data", main = "NYC Property Crimes - Differenced Lag 7")
abline(h=0, col='red')
acf(diff.nyc_p.tr, xlim=c(0.2,4), lag.max = 365 * 4, main = "NYC Property Crimes - Differenced Lag 7")
